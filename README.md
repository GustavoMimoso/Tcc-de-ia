# Tcc-de-ia
Um programa de Ia onde utiliza yolo para identificar gestos e fazer interacoes com uma interface
import cv2
...
# Definindo uma função para obter o caminho correto do arquivo
def resource_path(relative_path):
    """ Obter o caminho absoluto para o recurso, funcionando para desenvolvimento e execução como .exe """
    try:
        # PyInstaller cria um caminho temporário para armazenar os arquivos
...
# Inicializar o caminho para o arquivo de modelo
model_path = resource_path("models/hand_landmark_tracking_cpu.binarypb")
...
confirm_dialog_open = False  # Nova variável para controlar a abertura da caixa de diálogo
pending_gesture = None  # Armazena o gesto pendente para confirmação


# Função para verificar as travas e permitir apenas ações válidas com base no status atual
...
# Função para definir o novo status com base no gesto realizado
...
def mostrar_confirmacao(gesto):
    ...
    def confirmar():
        global status_atual  # Adicione esta linha para modificar a variável global

        if gesto == "Vendido":
            proximo_lote()  # Avançar para o próximo lote
            atualizar_descricao()  # Atualiza a descrição do novo lote
            status_atual = "Em Pregão"  # Atualiza o status após a venda
            update_status("Novo Lote")  # Define o status como "Novo Lote"
            gesture_history.clear()  # Limpa histórico, se necessário

        # Chame definir_status_por_gesto(gesto) agora
        definir_status_por_gesto(gesto)

        confirm_window.destroy()
        liberar_gesto()  # Libera para reconhecer novos gestos
...
# Função para liberar a detecção de gestos novamente após confirmar/cancelar
def liberar_gesto():
    global confirm_dialog_open, pending_gesture
...
# Funções dos gestos com validação e confirmação
def dou_lhe_uma():
...
lance_inicial = 1000  # Exemplo de valor inicial do lance
lances = []  # Lista para armazenar o histórico de lances

# Função para adicionar lances ao histórico
def adicionar_lance(valor):
    lances.append(valor)
    atualizar_historico()  # Atualiza a interface do histórico

# Função para atualizar o histórico de lances na interface
def atualizar_historico():
    historico_label.config(text="Histórico de lances:\n" + "\n".join(str(lance) for lance in lances))

# Função de incremento com histórico
def incremento_100():
    if verificar_gesto_valido("Incremento 100"):
        adicionar_lance(100)
        update_status("Incremento de R$ 100!")
...
def incremento_500():
    if verificar_gesto_valido("Incremento 500"):
        adicionar_lance(500)
        update_status("Incremento de R$ 500!")
# Verificar a orientação da palma da mão
...
# Detectar o gesto (mão fechada ou aberta)
            ...
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if verificar_orientacao_palmas(hand_landmarks):
                    ...
                    if current_time - last_gesture_time > gesture_hold_time and not confirm_dialog_open:
                        # Gesto de Próximo lote (polegar para cima)
                        if stable_fingers_up == [True, False, True, False, False]:
                            proximo_lote()
                        # Gesto de Anterior (polegar para baixo)
                        elif stable_fingers_up == [False, False, False, False, True]:
                            anterior_lote()
                        # Verificação para outros gestos já implementados
...
btn_anterior = ttk.Button(frame_nav, text="Anterior", command=lambda: print("Anterior"))
btn_anterior.pack(side="left", padx=10)
btn_proximo = ttk.Button(frame_nav, text="Próximo", command=lambda: print("Próximo"))
...
frame_descricao.place(relx=0, rely=0.14, relwidth=0.25, relheight=0.79)  # Aumentando a altura
...
frame_historico.place(relx=0.25, rely=0.14, relwidth=0.25, relheight=0.79)  # Aumentando a altura
...
frame_botoes_lances.place(relx=0.50, rely=0.14, relwidth=0.30, relheight=0.35)  # Aumentando a altura

# Configurando 3 colunas e 2 linhas
for i in range(3):
    frame_botoes_lances.grid_columnconfigure(i, weight=1)
for i in range(2):
    frame_botoes_lances.grid_rowconfigure(i, weight=1)

# Botões organizados em 2 linhas e 3 colunas
...
frame_incrementos.place(relx=0.80, rely=0.14, relwidth=0.15, relheight=0.35)  # Ajustando altura
...
for value in incremento_values:
    btn_incre = ttk.Button(frame_incrementos, text=f"R$: {value},00")
    btn_incre.pack(pady=10, padx=5, fill="x")

# Webcam label ajustado
webcam_label = tk.Label(root, bd=2, relief="solid", bg="black")  # Cor de fundo para o label da webcam
webcam_label.place(relx=0.50, rely=0.50, relwidth=0.45, relheight=0.43)  # Aumentando a área ocupada pela webcam
...
lotes = [
    ...
    },
    {
        ...
        "Lance inicial": 36.592,
        "Despesas": "Nada",
        "Mínimo": "R$ 40.500,00"

    }
    # Adicione mais lotes aqui se necessário
]
...
def atualizar_descricao():
    ...
    # Configurar a tag para texto em negrito
...
# Chame a função para atualizar a descrição do lote ao iniciar
atualizar_descricao()
